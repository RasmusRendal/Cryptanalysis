...
def decode_blocks(blocks, decrypt_func, block_len):
    ...
    decrypted_blocks = list(decrypted[i:block_len+i] for i in range(0, len(decrypted), block_len))
...
def modify_text(ciphertext, plaintext, decrypt_func, new_plaintext, block_len):
    ...
    blocks = list(ciphertext[i:block_len+i] for i in range(0, len(ciphertext), block_len))

    # Plaintext and the plaintext we want as blocks
    plain_blocks = list(plaintext[i:block_len+i] for i in range(0, len(plaintext), block_len))
    adv_blocks = list(new_plaintext[i:block_len+i] for i in range(0, len(new_plaintext), block_len))
    ...
    for i in range(1, len(plain_blocks)+1):
        ...
        new_enc[-1-i] = (xor.xor(xor.xor(adv_blocks[-i], plain_blocks[-i]), blocks[-1-i]))
